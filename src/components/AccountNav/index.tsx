...
export const AccountNav: React.FC<Props> = ({ className }) => {
  ...
  const navItems = [
    { href: '/account', label: t('homePortal'), icon: Castle },
    { href: '/account/addresses', label: t('coordinates'), icon: Compass },
    { href: '/orders', label: t('acquisitions'), icon: BookOpen },
    { href: '/account/reviews', label: t('whispers'), icon: Sparkles },
  ]
  ...
  return (
    <div className={cn('flex flex-col gap-1 w-full', className)}>
      ...
      >
        ...
        <span className="font-bold text-sm uppercase tracking-widest text-inherit">
          {t('banishSession')}
        </span>
      </Link>
    </div>
