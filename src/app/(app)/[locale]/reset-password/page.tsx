...
function ResetPasswordForm() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res.ok) {
                setStatus('success')
                setMessage('Your password has been reset. Redirecting to the portal...')
                setTimeout(() => router.push('/login'), 2500)
            } else {
                const data = await res.json().catch(() => ({}))
                setStatus('error')
                setMessage(data?.errors?.[0]?.message || 'Reset failed. Your link may have expired.')
            }
        } catch {
            setStatus('error')
    ...
    if (status === 'success') {
        return (
            <div className="flex flex-col items-center gap-4 py-12 text-center">
                ...
                <div>
                    <p className="text-xl font-bold text-[#4a3728] font-display">Secret Key Reset</p>
            ...
            <div className="flex justify-center pt-8 pb-10">
                ...
                >
                    ...
                    <div className="absolute -bottom-8 whitespace-nowrap text-[#4a3728] font-bold text-sm tracking-[0.2em] uppercase font-display">
                        {status === 'loading' ? 'Resetting...' : 'Update Key'}
                    </div>
                </button>
            </div>
...
export default function ResetPasswordPage() {
    return (
        <AuthLayout
            ...
            description="Cast a new secret key to regain access to the ancient library."
            sealText="Archivist's Seal"
        >
            <Suspense fallback={
